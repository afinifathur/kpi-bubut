...
# 🔒 Aturan Emas: Safe Production Deployment (KPI Bubut)
...
SSH ke server (`peroniks@peroniks-ppicserver`), lalu jalankan:

```bash
cd /srv/docker/apps/kpi-bubut

# Backup database sebelum pull
sudo docker compose exec db mysqldump -u root -p[PASSWORD] kpi_bubut > /home/peroniks/backups/kpi_backup_$(date +%Y%m%d_%H%M%S).sql
```
...
sudo docker compose exec -T db mysql -u root -p[PASSWORD] kpi_bubut < /home/peroniks/backups/kpi_backup_YYYYMMDD_HHMMSS.sql
