...
@section('content')
    ...
    <div class="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">KPI Harian Operator</h1>
        ...
        <div class="flex gap-2">
            {{-- PDF Filtered Export --}}
            <a href="{{ route('tracking.operator.pdf', request()->query()) }}" target="_blank" class="inline-flex items-center justify-center px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-medium rounded-lg transition-colors shadow-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2">
            ...
            <a href="{{ url('/export/operator/' . $endDate) }}" class="inline-flex items-center justify-center px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-lg transition-colors shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="p-4 border-b border-gray-100 bg-gray-50 flex flex-col md:flex-row gap-4 items-end justify-between">
            ...
            <form action="{{ route('api.manual.sync') }}" method="POST">
                @csrf
                {{-- Sync expects a 'date' param --}}
                <input type="hidden" name="date" value="{{ request('end_date', $endDate) }}">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($rows as $row)
                        <tr class="odd:bg-white even:bg-gray-50 hover:bg-blue-50 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4">
                                {{-- Shift info is tricky in range view, assume summary doesn't need distinct shift logic per row if rows are daily aggregates --}}
                                <span class="text-gray-400">-</span> 
                            </td>
                            <td class="px-6 py-4">
                                <div class="font-semibold text-gray-800">{{ $operatorNames[$row->operator_code] ?? $row->operator_code }}</div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function validateForm() {
            ...
            if (diffDays > 45) {
                Swal.fire({
                    icon: 'warning',
                    title: 'Batas Waktu',
                    text: 'Data yang bisa digenerate maksimal 45 hari per 1 operator. Silakan perkecil rentang tanggal.',
                });
                return false;
            }
        ...
        filterForm.addEventListener('submit', function(e) {
            ...
            // Check Operator
            if (isMultiDay && (!operatorSelect.value || operatorSelect.value === 'all')) {
                e.preventDefault();
                Swal.fire('Pilih Operator', 'Untuk rentang lebih dari 1 hari, Anda WAJIB memilih 1 operator spesifik.', 'warning');
                return;
            }
        });
    });
